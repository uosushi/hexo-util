...
function highlightUtil(str, options) {
  ...
  var before = useHljs ? '<pre class="code"><code class="hljs ' + options.lang + '">' : '<pre class="code">';
  ...
  for (var i = 0, len = lines.length; i < len; i++) {
    ...
    numbers += '<span class="line">' + '0'.repeat(3-(String(firstLine+i).length))+(firstLine + i) + '</span><br>';
  ...
  if (gutter) {
    result += '<pre class="gutter">' + numbers + '</pre>';
  }

  result += before + content + after;

  result+='</figure>'
  console.log("hexo-util/lib/highlight.js")
  return result;
}
